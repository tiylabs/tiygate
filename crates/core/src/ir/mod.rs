...
pub enum Content {
    ...
    Text {
        ...
        /// OpenAI GPT-5.6 explicit prompt-cache breakpoint attached to this
        /// exact content block. It must remain on the block to preserve the
        /// reusable-prefix boundary during Chat ↔ Responses conversion.
        #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    ToolCall {
        ...
        /// Original Responses/Codex wire item type when it is not a plain
        /// `function_call` (e.g. `custom_tool_call`, `local_shell_call`).
        /// Same-protocol Responses re-encode uses this to restore the wire
        /// shape; other protocols ignore it.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        wire_type: Option<String>,
    },
    ...
    ToolResult {
        ...
        caller: Option<ToolCaller>,
        /// Original Responses/Codex wire item type when it is not a plain
        /// `function_call_output` (e.g. `custom_tool_call_output`,
        /// `local_shell_call_output`). Same-protocol Responses re-encode uses
        /// this to restore the wire shape; other protocols ignore it.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        wire_type: Option<String>,
    },
    /// JavaScript program generated by Responses Programmatic Tool Calling.
    Program {
        id: String,
        call_id: String,
        code: String,
    ...
    /// Terminal result emitted by a Responses program item.
    ProgramOutput {
        id: String,
        call_id: String,
        result: String,
...
/// `"high"`, `"low"`, `"auto"`, `"original"`). Stored in
/// [`Content::Media::metadata`] so the
...
pub struct GenerationParams {
    ...
    /// Desired visible-answer verbosity for GPT-5-family models.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub verbosity: Option<Verbosity>,
}

/// Visible-answer verbosity requested from the model.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum Verbosity {
    Low,
...
///
/// Seven canonical levels that map across all protocols:
/// - **OpenAI Chat/Responses**: `reasoning_effort` / `reasoning.effort`
///   (none/minimal/low/medium/high/xhigh/max; GPT-5.6+ supports `max` natively)
...
impl ThinkingConfig {
    ...
    pub fn budget_to_effort(budget: u32) -> ThinkingEffort {
        match budget {
            0 => ThinkingEffort::None,
            1..=2047 => ThinkingEffort::Minimal,
...
mod tests {
    ...
    fn thinking_effort_serde_lowercase() {
        ...
        assert_eq!(
            serde_json::to_string(&ThinkingEffort::None).unwrap(),
            "\"none\""
        );
        assert_eq!(
        ...
        assert_eq!(
            serde_json::from_str::<ThinkingEffort>("\"none\"").unwrap(),
            ThinkingEffort::None
        );
        assert_eq!(
