...
pub enum Command {
    ...
    MigrateStatus,
    /// Upload and read back a small object through the payload archive S3 client.
    ArchiveCheck(ArchiveCheckArgs),
}

/// Arguments for the `archive-check` subcommand.
#[derive(Debug, Parser, Clone)]
pub struct ArchiveCheckArgs {
    /// Object key to write, relative to TIYGATE_PAYLOAD_ARCHIVE_S3_PREFIX.
    #[arg(long, default_value = "test.txt")]
    pub key: String,
    /// Text content to upload.
    #[arg(long, default_value = "tiygate archive connectivity test\n")]
    pub content: String,
}
...
mod tests {
    ...
    #[test]
    fn parse_archive_check_subcommand() {
        let args = Args::try_parse_from(["tiygate", "archive-check", "--key", "probe/test.txt"])
            .expect("parse");
        match args.command {
            Some(Command::ArchiveCheck(a)) => assert_eq!(a.key, "probe/test.txt"),
            _ => panic!("expected ArchiveCheck"),
        }
    }
