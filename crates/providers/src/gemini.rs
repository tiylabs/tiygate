...
pub fn apply_gemini_default_auth(
    ...
    upstream_headers.insert(
        http::HeaderName::from_static(GEMINI_API_KEY_HEADER),
        hv,
    );
    Ok(())
}
...
mod tests {
    ...
    fn test_append_api_key_query_preserves_existing_query() {
        ...
        .unwrap();
        assert!(u.contains("alt=sse"), "alt=sse should be preserved, got {u}");
        assert!(u.contains("&key=AIza-abc"), "key should be appended with &, got {u}");
        assert!(!u.contains("?key"), "must not use ? when alt= present, got {u}");
    }
