...
> 实现入口：`webui/src/index.css`（token）、`webui/src/components/ui/`（组件）、`webui/src/lib/theme.tsx`（主题切换）
...
| Token | Light | Dark | 用途 |
| --- | --- | --- | --- |
| `--bg` | `#F7F8FA` | `#0B0D12` | 页面背景 |
| `--surface` | `#FFFFFF` | `#12151C` | 卡片、表格、弹窗 |
| `--surface-muted` | `#F1F3F6` | `#1A1E27` | 次级区域、表头、hover 背景 |
| `--border` | `#E4E7EC` | `#262B36` | 默认边框、分割线（hairline） |
| `--border-strong` | `#D0D5DD` | `#343B49` | 输入框、可交互控件边框 |
| `--text` | `#101828` | `#EDEFF3` | 主文本 |
| `--text-muted` | `#475467` | `#A4ACBB` | 次级文本 |
| `--text-subtle` | `#667085` | `#767E8E` | 说明文本、占位符 |
...
| `--accent` | `#E56910` | `#F08C3D` | 一次性重要操作（复制 secret 等），非危险但需注意 |
| `--accent-strong` | `#C45708` | `#F6A86A` | accent hover |

### 3.3 状态色（每色三档：default / strong / soft）

| Token | Light | Dark | 用途 |
| --- | --- | --- | --- |
| `--success` | `#17914F` | `#45C77F` | healthy、enabled、ok |
...
落地位置：`webui/src/index.css`。通过 Tailwind v4 `@theme inline` 映射为工具类（`bg-surface`、`text-text-muted` 等），暗色经 `[data-theme="dark"]` 切换（`lib/theme.tsx`）。
