...
| **上游** | 常规非流式总超时、流式 idle / total / TTFB 超时、TCP keepalive、连接池 idle 超时、TCP nodelay | `TIYGATE_UPSTREAM_NONSTREAM_TIMEOUT_SECS`、`TIYGATE_UPSTREAM_STREAM_IDLE_TIMEOUT_SECS`、`TIYGATE_UPSTREAM_STREAM_TOTAL_TIMEOUT_SECS`、`TIYGATE_UPSTREAM_TTFB_TIMEOUT_SECS`、`TIYGATE_UPSTREAM_TCP_KEEPALIVE_SECS`、`TIYGATE_UPSTREAM_POOL_IDLE_TIMEOUT_SECS`、`TIYGATE_UPSTREAM_TCP_NODELAY` |
